...
export default function CoachDashboard() {
  ...
  <CoachExtraCharts clients={coach.clients} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          {/* Ajout séance coach moderne */}
          <div className="col-span-1 md:col-span-3">
            <div className="space-y-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-gray-700 mt-6">
              ...
              <form onSubmit={e => { e.preventDefault(); /* handleAddSession() */ }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nom du programme *</label>
                    <input type="text" value={programName} onChange={e => setProgramName(e.target.value)} placeholder="Ex: trql" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" required />
                  </div>
                </div>
                <div className="mt-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Titre de la séance (optionnel)</label>
                  <input type="text" value={sessionTitle} onChange={e => setSessionTitle(e.target.value)} placeholder="Ex: Séance Pecs - Fente + HIIT" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                </div>
                ...
                <div className="mt-4 mb-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => runSearch(searchQuery)} className="px-3 py-2 bg-blue-600 text-white rounded">{isSearching ? 'Recherche...' : 'Rechercher'}</button>
                  </div>
                    ...
                    <div className="mt-3 space-y-2">
                      {searchResults.map((ex, i) => (
                        <div key={i} className="p-3 bg-white dark:bg-gray-800 rounded border flex items-center justify-between">
                          ...
                          <div>
                            <button type="button" onClick={() => addExternalExerciseToList(ex)} className="px-3 py-1 bg-green-600 text-white rounded">Ajouter</button>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="mt-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Exercices ajoutés</label>
                  {addedExercises.length > 0 ? (
                    <ul className="space-y-2">
                      {addedExercises.map((ex, i) => (
                        <li key={i} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded px-3 py-2">
                          <span>{ex.name || ex.exercise || ex.title}</span>
                          <button type="button" onClick={() => removeExercise(i)} className="text-red-500 hover:text-red-700 text-xs">Retirer</button>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <div className="text-xs italic text-gray-500">Aucun exercice détecté</div>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Durée (minutes) *</label>
                    <input type="number" value={duration} onChange={e => setDuration(e.target.value)} placeholder="Ex: 45" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nombre d'exercices *</label>
                    <input type="number" value={addedExercises.length} readOnly className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                  </div>
                </div>
                <div className="mt-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes (optionnel)</label>
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Commentaires sur la séance, difficultés rencontrées, remarques..." rows={2} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                </div>
                <div className="flex justify-end gap-3 mt-6">
                  <button type="button" onClick={resetForm} className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500">Annuler</button>
                  <button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Ajouter la séance</button>
                </div>
              </form>
            </div>
          </div>
